...
1) added getStore(), which retrieves the reference to the underlying store.
2) added new event 'selected' when any item is moved to selected bounded list
3) changed height for each bounded list to 150
...
Ext.define('Ext.ux.form.ItemSelector', {
        ...
        var me = this,
            ...
            toConfig = Ext.apply({
                ...
                store: Ext.create('Ext.data.Store', { model: me.store.model ,
                	listeners: // items are selected when items are added to the 'to' store
                		{
                			add: function(store,records,index) { 
                 				if(!Ext.isEmpty(records)) {
                  					me.fireEvent('select',store,records,index);		
                 				}
                			},
                			remove: function(store,record,index) {
                				me.fireEvent('remove',store,record,index);
                			},
    ...
    clear: function() {
    	fromStore.removeAll(false);
    	toStore.removeAll(false);
    },
